...
public class WebCalcLoad extends BaseTestClass {
	...
	public static WebDriver driver;
	  ...
	  public WebCalcLoad() { synchronized (this) { 
		  // This initElements method will create all WebElements 
		  PageFactory.initElements(DriverManager.getDriver(), this); //this.driver = DriverManager.getDriver();
	...
	public void LaunchApplication() {
		Reporter.logIntoReport(LogStatus.INFO, "User has launched Calcualtor application");

		try {
			if (DriverManager.getDriver().getTitle() != null
					&& DriverManager.getDriver().getTitle().equalsIgnoreCase("Calculator")) {
			...
			Reporter.logIntoReport(false, true, "Error in LaunchApplication test :" + e.getMessage(), "");
		}
	...
	public void LaunchApplication2() {
		...
		try {
			if (DriverManager.getDriver().getCurrentUrl() != null
					&& DriverManager.getDriver().getCurrentUrl().equalsIgnoreCase("Calculator")) {
				Reporter.logIntoReport(true, true, "", " Launched Successfully");
				// Utils.takeScreenshot("Home Page", "Home Page displayed");
			} else {
				Reporter.logIntoReport(false, true, " calculator is not launched", "");
	...
	public static double validateOperations(String Operation, Object number1, Object number2) {
		...
		try {
			switch (Operation) {
			case "+":
				Reporter.logIntoReport(LogStatus.INFO, "First number is : " + firstNum.getAttribute("value")
						+ " Second Number is : " + secondNum.getAttribute("value"));
	...
	public void validateResult(Object obj1, Object obj2) {
		...
		if (obj1.toString().equals(obj2.toString())) {
			flag = true;
			Reporter.logIntoReport(flag, true, " Expected value is : " + obj1 + " Actual value is : " + obj2,
					" Actual value is : " + obj1 + " Expected value is : " + obj2);
		}

		else {
			flag = false;
			Reporter.logIntoReport(flag, true, " Actual value is : " + obj1 + " Expected value is : " + obj2,
					" Expected value is : " + obj1 + " Actual value is : " + obj2);
		}
	}
